...
# LPJ reflectance data ------------------------------------------- 
    lpj = xr.open_dataset(reflectance_file, decode_times=False).sel(time=month-1)
...
# output ENVI --------------------------------------------
    if outfiletype == 'ENVI': # if ENVI output file is desired
        rdn_out = join(output_dir, 'lpj-prosail_levelD_TOA-radiance_' + version[:-2]+'3'+version[-1:] + '_m_' + year +'.nc')
        envi.save_image(rdn_out, lpj_rdn);

        if modei == 'hypertrace':
            rtr_out = join(output_dir, 'Retrievals', 'lpj-prosail_levelE_retrieved-HDR_' + version[:-2]+'3'+version[-1:] + '_m_' + year +'.nc');
...
# output ncdf -----------------------------------------------
        ...
        if mode =='hypertrace':
            rtr_out = join(output_dir, 'Retrievals', 'lpj-prosail_levelE_retrieved-HDR_' + version[:-2]+'3'+version[-1:] + '_m_' + year +'.nc');
            rtr_out_nc = nc.Dataset(join(hyperSims, 'data', rtr_out), 'a');
            ...
            rtr_var = rtr_out_nc.createVariable('Radiance','float32',('lat','lon','wavelength'));
            rtr_var.setncattr('units','W sr-1 m-2');
            rtr_var[:] = lpj_rtr; 
            rtr_out_nc.close();        
